      ...
      [key]: str
        ...
        # If type == 'scalar'...
          value: (int|float|bool|str|list|dict|set)
            # NOTE: If specify a 'set', then need to prepend the set with the '!!set' tag
    ...
    states: dict
      [key]: str
        ...
        resource_deltas (+): dict[float] (default = {})
          # [key] = resource from 'variables', [value] = how much to change each resource level AS SOON AS this state is hit
        ...
        transitions (+): list[dict] (default = [])
            ...
            # - The first half of transitions have an 'if' condition, but the second have of transitions have a 'prob'
              if (+): bool|str (default = bool:true)
                ...
                # Conditionals will be evaluated in order and break on first TRUE
            ...
            utilities (+): str|float|bool|list[dict] (default = 0.0)
              # Same as for state
            resource_deltas (+): dict[float] (default = {})
              # [key] = resource from 'variables', [value] = how much to change each resource level by taking this transition